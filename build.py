...
def main():
    if len(sys.argv) < 4:
        print(
            f"Usage: {sys.argv[0]} <input.toml> <output.html> <template.html>",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Use directory name as fallback for canonical_name
    if not canonical_name:
        import os

        canonical_name = os.path.basename(os.path.dirname(input_toml))
